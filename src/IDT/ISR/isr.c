...
void __attribute__((cdecl)) i686_ISR_Handler(Registers* regs)
{
    ...
    {
        new_line();
        print_string("[ISR] Unhandled interrupt");
    }
    else
    {
        panic_clear_screen();
        panic_print_string("KERNEL PANIC \n");
        panic_print_string("KERNEL PANIC \n");
        panic_print_string("KERNEL PANIC \n");
        panic_print_string("KERNEL PANIC \n \n");
        panic_print_string("[ISR] Unhandled exception ");
        panic_print_string(g_Exceptions[regs->interrupt]);
        i686_Panic();
    }
}